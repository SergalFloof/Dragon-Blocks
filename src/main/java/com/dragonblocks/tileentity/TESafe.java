package com.dragonblocks.tileentity;
...
import com.dragonblocks.data.Safe;
import net.minecraft.tileentity.TileEntity;
import net.minecraft.util.EnumFacing;

public class TESafe extends TEBase implements ISidedInventory {
    ...
    public void updateEntity()
    {
        if (!worldObj.isRemote) {            
            ...
            if (contentsChanged && (tickCount % 20 == 0)) {
                worldObj.markBlockForUpdate(pos, yCoord, zCoord);
    ...
    {
        TileEntity tileEntity = entityPlayer.getEntityWorld().getTileEntity(pos, yCoord, zCoord);

        if(tileEntity != null && tileEntity.equals(this)) {
            return entityPlayer.getDistanceSq(pos + 0.5D, yCoord + 0.5D, zCoord + 0.5D) <= 64.0D;
        }
    ...
    public boolean receiveClientEvent(int eventId, int eventArg)
    {
        if (eventId == EVENT_ID_STATE_CHANGE) {
            ...
            world.playSound((double)pos + 0.5D, (double)yCoord + 0.5D, (double)zCoord + 0.5D, soundName, 1.0F, worldObj.rand.nextFloat() * 0.1F + 0.9F, false);
            stateChanged = true;
            return true;
        } else {
    ...
    public void openInventory(EntityPlayer player) {
    	world.addBlockEvent(this.pos, this.getBlockType(), EVENT_ID_STATE_CHANGE, Safe.STATE_OPEN);
    ...
    {
        world.addBlockEvent(this.pos, this.getBlockType(), EVENT_ID_STATE_CHANGE, Safe.STATE_CLOSED);
    }
    ...
    @Override
    public boolean canInsertItem(int side, ItemStack itemStackIn, EnumFacing direction) {
    	return Safe.allowsInsertion(this) && Safe.getFacing(this).ordinal() != side;
    }
    @Override
    public boolean canExtractItem(int side, ItemStack stack, EnumFacing direction) {
