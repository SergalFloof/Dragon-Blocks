...
import com.dragonblocks.block.BlockCarpentersSafe;
import com.dragonblocks.data.Safe;
import com.dragonblocks.tileentity.TESafe;
...
public class BlockHandlerSafe extends BlockHandlerBase {
    ...
    protected void renderCarpentersBlock(int x, int y, int z)
    {
        ...
        if (TE.hasAttribute(TE.ATTR_UPGRADE)) {
            String name = BlockProperties.getOreDictMatch(TE.getAttribute(TE.ATTR_UPGRADE), BlockSafe.upgradeOres);
            if (!"".equals(name)) {
                for (int idx = 0; idx < BlockSafe.upgradeOres.length; ++idx) {
                    if (BlockSafe.upgradeOres[idx].equals(name)) {
                        panelItemStack = BlockSafe.upgradeStack[idx];
                    }
                }
            }
        } else {
