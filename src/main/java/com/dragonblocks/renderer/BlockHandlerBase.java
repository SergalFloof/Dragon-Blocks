...
import com.dragonblocks.renderer.helper.LightingHelper;
...
public class BlockHandlerBase implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess blockAccess, int x, int y, int z, Block block, int modelID, RenderBlocks renderBlocks)
    {
        ...
        if (TE_default != null && TE_default instanceof TEBase) {
            ...
            if (FeatureRegistry.enableRailSlopes)
            {
                ...
                if (blockAccess.isSideSolid(x, y, z, EnumFacing.UP, false) && blockYP instanceof BlockRailBase)
                {
                    int metadata = ((BlockRailBase)blockYP).getBasicRailMetadata(blockAccess, null, x, y + 1, z);

                    BlockHandlerSlope slopeHandler = new BlockHandlerSlope();
     ...
     */
    protected void renderBlockWithRotation(ItemStack itemStack, int x, int y, int z, double xMin, double yMin, double zMin, double xMax, double yMax, double zMax, EnumFacing ... dir)
    {
     ...
     */
    protected void renderPane(IIcon icon, int x, int y, int z, EnumFacing facing, boolean enableAO, boolean flipped)
    {
     ...
     * @param facing
     */
    private void doLightAndRenderSide( int x, int y, int z, EnumFacing facing, boolean enableAO)
    {
